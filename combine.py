...
plt.xlim(xMin, xMax)
ticks = np.linspace(xMin, xMax, 21) #9)
plt.xticks(ticks, [round(100*t,2) if i%5==0 else '' \
                   for i,t in enumerate(ticks)],size=medFont)

plt.ylabel(f'wt% {e2}', size=bigFont)
plt.ylim(yMin, yMax)
ticks = np.linspace(yMin, yMax, 21) #8
plt.yticks(ticks,[round(t*100,2) if i%5==0 else '' \
                  for i,t in enumerate(ticks)], size=medFont)
